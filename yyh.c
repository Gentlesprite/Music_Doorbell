
// AT89S52+11.0592MHz+无源蜂鸣器
// 共三个按键:叮咚门铃按钮、音乐门铃按钮、下一曲按键
//v0.4:对代码进行优化
#include "REG52.H"
#include "lcd1602.h"
typedef unsigned char u8; // 定义无符号8位整型变量
typedef unsigned int u16; // 定义无符号16位整型变量
sbit doorbell = P3 ^ 2;   // 门铃int0
sbit play_down = P3 ^ 4;  // 下一首歌
sbit pause = P3 ^ 3;      // 播放\暂停int1
sbit speaker = P2 ^ 0;    // 无源蜂鸣器
sbit led = P1 ^ 0;

unsigned char timer0h, timer0l, time; // timer0h,timer0l为定时器T0的高低位初值,time为对应的节拍时间
unsigned char music_num; // music_num为歌曲编号,music_num=0表示刚开机时的状态,num是查找歌曲数据表的地址
unsigned char press_count;
unsigned int lcd_change;
unsigned char doorbell_signal;
// unsigned int num;
signed int fre;               // 对应频率数据表的地址
bit play_enable;              // 歌曲播放的使能标志位,用于播放\暂停
void DelayBeat(unsigned char t);  // 延时子函数,控制发音的时间长度
void DelayMs(unsigned int t); // 普通延时子程序,可用于按键消抖
void PlayANote(void);              // 演奏一个音符
void music_play(void);        // 播放歌曲


// 每三个数字,代表一个音符
// 第一个数字是音符的数值1234567之一(第几个音),代表哆来咪发...
// 第二个数字是0123之一,代表低音\中音\高音\超高音(第几个八度)
// 第三个数字是时间长度,以半拍为单位,乐曲数据表的结尾是三个0



unsigned char code song_doorbell[] = {3, 3, 1, 1, 3, 2, 0, 0, 0};
//《世上只有妈妈好》
unsigned char code song1[] = {6, 2, 3, 5, 2, 1, 3, 2, 2, 5, 2, 2, 1, 3, 2, 6, 2, 1,
                              // 6,2,3代表6,中音,3个半拍;
                              // 5,2,1代表5,中音,1个半拍;
                              // 3,2,2代表3,中音,2个半拍;
                              // 5,2,2代表5,中音,2个半拍;
                              // 1,3,2代表1,高音,2个半拍;
                              //...
                              5, 2, 1, 6, 2, 4, 3, 2, 2, 5, 2, 1, 6, 2, 1, 5, 2, 2, 3, 2, 2, 1, 2, 1, 6, 1, 1, 5, 2, 1,
                              3, 2, 1, 2, 2, 4, 2, 2, 3, 3, 2, 1, 5, 2, 2, 5, 2, 1, 6, 2, 1, 3, 2, 2, 2, 2, 2, 1, 2, 4,
                              5, 2, 3, 3, 2, 1, 2, 2, 1, 1, 2, 1, 6, 1, 1, 1, 2, 1, 5, 1, 6, 0, 0, 0};



// 《当你孤单你会想起谁》
unsigned char code song2[] = {3, 2, 2, 3, 2, 1, 4, 2, 1, 3, 2, 2, 2, 2, 1, 1, 2, 1, 2, 2, 2, 5, 2, 2, 2,
                              2, 2, 2, 2, 3, 1, 2, 2, 1, 2, 1, 2, 2, 1, 1, 2, 2, 7, 1, 1, 6, 1, 1, 7, 1,
                              2, 3, 2, 2, 7, 1, 2, 7, 1, 3, 6, 1, 2, 2, 2, 1, 3, 2, 1, 2, 2, 1, 1, 2, 1,
                              6, 1, 2, 5, 1, 2, 2, 2, 1, 3, 2, 1, 2, 2, 1, 1, 2, 1, 6, 1, 2, 6, 1, 2, 2,
                              2, 1, 3, 2, 1, 2, 2, 1, 1, 2, 1, 6, 1, 1, 7, 1, 1, 1, 2, 6, 0, 0, 0};

// 《送别》
unsigned char code song3[] = {5, 1, 2, 3, 1, 3, 5, 1, 1, 1, 2, 3, 6, 1, 2, 1, 2, 2, 5, 1, 4, 5, 1, 2, 1, 1, 1, 2, 1, 1,
                              3, 1, 2, 2, 1, 1, 1, 1, 1, 2, 1, 4, 5, 1, 2, 3, 1, 1, 5, 1, 1, 1, 2, 2, 7, 1, 1, 6, 1, 2,
                              1, 2, 2, 5, 1, 4, 5, 1, 2, 2, 1, 1, 3, 1, 1, 4, 1, 2, 7, 0, 1, 1, 1, 4, 6, 1, 2, 1, 2, 2,
                              1, 2, 4, 7, 1, 2, 6, 1, 1, 7, 1, 1, 1, 2, 4, 6, 1, 1, 7, 1, 1, 1, 2, 1, 6, 1, 1, 6, 1, 1,
                              5, 1, 1, 3, 1, 1, 1, 1, 1, 2, 1, 8, 5, 1, 2, 3, 1, 1, 5, 1, 1, 1, 2, 2, 7, 1, 1, 6, 1, 2,
                              1, 2, 2, 5, 1, 4, 5, 1, 2, 2, 1, 1, 3, 1, 1, 4, 1, 2, 7, 0, 2, 1, 1, 4, 0, 0, 0};

// 频率-半周期数据表 高八位  共保存了四个八度的28个频率数据
unsigned char code FREQH[] = {0xF2, 0xF3, 0xF5, 0xF5, 0xF6, 0xF7, 0xF8,       // 低音1234567
                              0xF9, 0xF9, 0xFA, 0xFA, 0xFB, 0xFB, 0xFC, 0xFC, // 1,2,3,4,5,6,7,i
                              0xFC, 0xFD, 0xFD, 0xFD, 0xFD, 0xFE,             // 高音 234567
                              0xFE, 0xFE, 0xFE, 0xFE, 0xFE, 0xFE, 0xFF};      // 超高音 1234567

// 频率-半周期数据表 低八位
unsigned char code FREQL[] = {0x42, 0xC1, 0x17, 0xB6, 0xD0, 0xD1, 0xB6,       // 低音1234567
                              0x21, 0xE1, 0x8C, 0xD8, 0x68, 0xE9, 0x5B, 0x8F, // 1,2,3,4,5,6,7,i
                              0xEE, 0x44, 0x6B, 0xB4, 0xF4, 0x2D,             // 高音 234567
                              0x47, 0x77, 0xA2, 0xB6, 0xDA, 0xFA, 0x16};      // 超高音 1234567


void LcdFirstChange(void)
{
    if (lcd_change == 0)
    {
        write_num(11, music_num, 1);

    }
}
void LcdSecondChange(void)
{
    write_num(13, press_count, 2);
}


void StopMusic(void)
{
    play_enable = ~play_enable;
    speaker = 1;
}

void PlayANote(void) // 演奏一个音符
{
    TH0 = timer0h; // 控制音调
    TL0 = timer0l;
    TR0 = 1;     // 启动T0, 由T0输出方波发音
    DelayBeat(time); // 每个音符的演奏时间
}


void main(void)
{
    init1602();
    EA = 1; // CPU开中断
    ET0 = 1;
    ET1 = 1;
    // time0init();   // 调用定时器0初始化函数
    TMOD = 0x11; // T0 T1 均在工作方式1
    lcd_change = 1;
    lcd1602_write_character(0, 1, " Welcome to Use");
    lcd1602_write_character(0, 2, " Press count:00 "); // 门铃标号

    while (1)
    {
        music_play(); // 根据当前状态播放相应歌曲的某个音符
        LcdFirstChange();
        LcdSecondChange();

        if (!pause) // 暂停键处理
        {
            DelayMs(1);
            if (!pause)
            {   
                lcd_change = 0;
                doorbell_signal = 0;
                lcd1602_write_character(0, 1, "  Song num:     ");
                DelayMs(5);
                LcdFirstChange();
                press_count++;
                
                if (music_num == 0) // music_num=0只有在刚开机,且未按下暂停键时存在,表示刚开机时的状态,按下后从第一首开始播放
                {
                    lcd_change = 0;
                    lcd1602_write_character(0, 1, "  Song num:01   ");
                    // press_count++;
                    doorbell_signal = 0;
                    music_num = 1;   // 歌曲序号置1
                    num = 0;         // 从头播放
                    play_enable = 1; // 允许播放
                }
                else
                {
                    StopMusic();
                }
                while (!pause) // 若按着暂停键不放手时的处理
                {
                    if (play_enable == 0)
                    {
                    } // 如果是暂停，则显示时间不变//（暂停时play_enable==0）
                }

            } // 暂停键处理结束

        } // while结束
        if (doorbell == 0)
        {
            DelayMs(1);
            if (doorbell == 0)
                lcd_change = 1;
            {
                lcd1602_write_character(0, 1, " Door bell mode");
                num = 0;
                doorbell_signal = 1;
                play_enable = 1; // 允许播放
                press_count++;
                while (doorbell == 0) // 若按着暂停键不放手时的处理
                {
                    if (play_enable == 0)
                    {
                        lcd_change = 0;
                    } // 如果是暂停，则显示时间不变//（暂停时play_enable==0）
                }
            }
        }

        if ((!play_down) && (music_num != 0))
        {
            DelayMs(1);
            if ((!play_down) && (music_num != 0))
            {
                speaker = 1;
                press_count++;
                music_num += 1; // 歌曲编号加一
                if (music_num >= 4)
                    music_num = 1;
                num = 0; // 从头开始播放

                DelayMs(500); // 歌曲切换时延时0.5S
            }
        }
    }
}

void DelayMs(unsigned int t) // MS延时子程序
{
    unsigned int i, j;
    for (i = 0; i < t; i++)
    {
        for (j = 0; j < 123; j++)
            ;
    }
}

void DelayBeat(unsigned char t) // 延时子函数,控制发音的时间长度,每个节拍0.4S
{
    unsigned char t1;
    unsigned long t2;
    for (t1 = 0; t1 < t; t1++) // 嵌套循环, 共延时t个半拍
    {
        for (t2 = 0; t2 < 6500; t2++) // 延时期间, 可进入T0中断去发音
        {
            ;
        }
    }
    TR0 = 0; // 关闭T0, 停止发音
}

void timer0(void) interrupt 1 // T0中断程序,控制发音的音调
{
    speaker = !speaker; // 输出方波, 发音
    TH0 = timer0h;      // 下次的中断时间,这个时间控制音调高低
    TL0 = timer0l;
}



void music_play(void) // 播放相应歌曲的某个音符
{
    if ((music_num == 1) && (play_enable == 1) && (doorbell_signal == 0))
    {   
        
        fre = song1[num] + 7 * song1[num + 1] - 1; // 第i个是音符,第n+1个是第几个八度
        timer0h = FREQH[fre]; // 从数据表中读出频率数值,实际上是定时的时间长度
        timer0l = FREQL[fre];
        time = song1[num + 2]; // 读出时间长度数值
        num += 3;
        if (fre < 0) // 判断歌曲的结束位,结束后转到下一首
        {
            num = 0; // 下一首从头播放
            lcd_change = 1;
            music_num = 2;
            StopMusic();
        }
        PlayANote();
    }

    if ((music_num == 2) && (play_enable == 1) && (doorbell_signal == 0))
    {   
        
        fre = song2[num] + 7 * song2[num + 1] - 1;
        timer0h = FREQH[fre];
        timer0l = FREQL[fre];
        time = song2[num + 2];
        num += 3;
        if (fre < 0)
        {
            num = 0;
            lcd_change = 1;
            music_num = 3;
            StopMusic();
        }
        PlayANote();
    }

    if ((music_num == 3) && (play_enable == 1) && (doorbell_signal == 0))
    {
        
        fre = song3[num] + 7 * song3[num + 1] - 1;
        timer0h = FREQH[fre];
        timer0l = FREQL[fre];
        time = song3[num + 2];
        num += 3;
        if (fre < 0)
        {
            num = 0;
            lcd_change = 1;
            music_num = 1;
            StopMusic();
        }
        PlayANote();
    }

    if ((doorbell_signal == 1) && (play_enable == 1))
    {
        fre = song_doorbell[num] + 7 * song_doorbell[num + 1] - 1;
        timer0h = FREQH[fre];
        timer0l = FREQL[fre];
        time = song_doorbell[num + 2];
        num += 3;

        if (fre < 0)
        {
            num = 0;
            StopMusic();
        }
        PlayANote();
    }
}
